...
from database import (db_create_todo, db_delete_todo, db_get_single_todo,
                      db_get_todos, db_update_todo)
...
async def update_todo(request: Request, response: Response,
                      id: str, data: TodoBody,
                      csrf_protect: CsrfProtect = Depends()):
    new_token = auth.verify_csrf_update_jwt(
        request, csrf_protect, request.headers)
    todo = jsonable_encoder(data)
