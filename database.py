...
async def db_signup(data: dict) -> dict:
    ...
    overlap_user = await collenction_user.find_one({"email": email})
    if overlap_user:
        raise HTTPException(status_code=400, detail="Email is already taken.")
    if not password or len(password) < 6:
        raise HTTPException(status_code=400, detail="Password too short.")
    user = await collenction_user.insert_one({
        "email": email,
        "password": auth.generate_hashed_pw(password)
    })
    new_user = await collenction_user.find_one({"_id": user.inserted_id})
    if new_user:
...
async def db_login(data: dict) -> str:
    email = data.get("email")
    password = data.get("password")
    user = await collenction_user.find_one({"email": email})
